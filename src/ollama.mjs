...
export async function adaptToModel(cfg) {
  ...
  //   何が悪いのか分かりにくくなるため。
  //
  // ■ 何が起きるか（2026-09-12 実機で測った）
  //   ollama（0.32.1）は大きすぎる num_ctx を **黙ってモデルの上限に詰めて 200 を返す。**
  //   詰め先は「そのモデルの上限」で、OLLAMA_CONTEXT_LENGTH ではない。
  //     qwen3-embedding:0.6b   999,999,999 → 32,768
  //     qwen2.5-coder:14b      999,999,999 → 32,768（会話の経路）
  //   llama-server のログ `new slot, n_ctx = 32768` で実際の広さを確認した。
  //
  //   **だから困るのは「遅くなる」ことではなく、こちらの数字だけが嘘になること。**
  //   ・圧縮のしきい値は numCtx × compactAtRatio（agent.mjs）なので、
  //     10億を渡すとしきい値も10億になり、**圧縮が一度も走らないまま**
  //     モデル側の窓（実際は上限ぶん）が埋まる。
  //   ・ほかのセッションと違う広さで呼ぶと、ollama は同じモデルを積み直す。
  //     2026-09-10 に model-guard が捕まえた「10分で7回積み直し」がこれ。
  //
  // ■ 以前ここに書いていた「毎ターン43秒」は誤り（2026-09-12 に訂正）
  //   5xx が返れば確かに retryWaitsMs [3000, 10000, 30000] で43秒待つ
  //   （偽サーバに向けた実測で 43.21 秒）。だが **num_ctx 過大では 5xx が返らない**ので、
  //   この経路には入らない。43秒が出るのは積み直し・降ろされ（"unexpected EOF"）のとき。
  //
  // ■ 上限は最初から手元にあった
  ...
  if (Number.isFinite(上限) && 上限 > 0 && Number(cfg.numCtx) > 上限) {
    notes.push({
      ...
      text:
        ...
        `そのまま起動しますが、ollama はこの値を黙って ${上限.toLocaleString()} に詰めます。` +
        'こちら側の数字だけが大きいままなので、文脈を短くする判断が効かなくなり、' +
        'ほかのセッションと違う広さで呼ぶとモデルの積み直しも起きます' +
        `（--ctx か ~/.qwythos-code/config.json の numCtx を ${上限.toLocaleString()} 以下に）。`
    });
  }
